...
C 是各個作業未及時完成(一定時間內)處罰的參數且長度為N的陣列
C2 是各個作業未及時完成(一定時間後)處罰的參數且長度為N的陣列
...
'''
    ...
    n = len(P)
    m = LpProblem("task scheduling", LpMinimize)
    ...
    m += lpSum(C[i]*t[i]+C2[i]*(t2[i]-TH[i]) for i in range(n)), "Total Cost of Lateness" 
    ...
    for i in range(n):
        for j in range(i+1,n):
            m += x[i] + P[j-(i+1)] - x[j] <= M*z[i][j-(i+1)], f"seq_constrain_{i+1}_{j+1}"
